					...
					<table class="table table-hover">
						...
						<tbody>
							<c:forEach var="post" items="${boastPosts}" varStatus="status">
								<c:set var="num" value="${fn:length(boastPosts)-status.index}" />
								<c:if
									test="${num>fn:length(boastPosts)-10*pg && num<=fn:length(boastPosts)-10*(pg-1)}">
					...
					<div class="text-center" style="margin-top: 100px;">
						...
						<ul class="pagination">
							...
							<fmt:parseNumber var="totalPg"
								value="${fn:length(boastPosts)/10+(1-((fn:length(boastPosts)/10)%1))%1}"
								integerOnly="true" />
							...
							<c:if test="${paginationNum ne 1}">
								<li class="page-item"><a class="page-link"
									href="/page/boastBoard?pg=${paginationPrevious}">Previous</a></li>
							</c:if>
							<c:forEach var="i" begin="1" end="${totalPg}" step="3">
								<c:if test="${i>(paginationNum-1)*3 && (i+2)<=paginationNum*3}">
									<li class="page-item <c:if test="${pg eq i}">active</c:if>"><a
										class="page-link" href="/page/boastBoard?pg=${i}">${i}</a></li>
									<c:if test="${(i+1)<=totalPg}">
										<li
											class="page-item <c:if test="${pg eq (i+1)}">active</c:if>"><a
											class="page-link" href="/page/boastBoard?pg=${i+1}">${i+1}</a></li>
									</c:if>
									<c:if test="${(i+2)<=totalPg}">
									<li class="page-item <c:if test="${pg eq (i+2)}">active</c:if>"><a
										class="page-link" href="/page/boastBoard?pg=${i+2}">${i+2}</a></li>
									</c:if>
								</c:if>
							</c:forEach>
							...
							<c:if test="${(paginationNum ne paginationTotal)&& (paginationTotal ne 0)}">
								<li class="page-item"><a class="page-link"
									href="/page/boastBoard?pg=${paginationNext}">Next</a></li>
							</c:if>
						</ul>
