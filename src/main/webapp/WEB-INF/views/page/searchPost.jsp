										...
										<tr>
											...
											<td><c:if test="${fn:contains(theString,'관리자')}">${singeFinded.user.nickname}</c:if>
												<c:if test="${fn:indexOf(theString,'관리자')==-1}">
													<a href="user?user_idx=${singeFinded.user.user_idx}&kara_type=0&sort=0"
									...
									<c:if test="${i>(paginationNum-1)*3 && (i+2)<=paginationNum*3}">
										...
										<c:if test="${(i+1)<=totalPg}">
											<li
												...
												class="page-link" href="/page/searchPost?pg=${i+1}&search_type=${type}&keyword=${keyword}">${i+1}</a></li>
										</c:if>
										<c:if test="${(i+2)<=totalPg}">
											<li
												class="page-item <c:if test="${pg eq (i+2)}">active</c:if>"><a
												class="page-link" href="/page/searchPost?pg=${i+2}&search_type=${type}&keyword=${keyword}">${i+2}</a></li>
										</c:if>
									</c:if>
